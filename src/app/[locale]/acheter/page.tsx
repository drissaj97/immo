...
export default async function AcheterPage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  const { items } = await searchListings({ transactionType: "sale", limit: 24 });
  return (
    <div className="mx-auto max-w-7xl px-4 py-8 lg:px-8">
      <h1 className="font-serif text-3xl">Acheter</h1>
