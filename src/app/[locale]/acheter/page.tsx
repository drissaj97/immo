...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return buildMetadata({
    ...
    description: "Villas, appartements et terrains à vendre.",
    path: "/acheter",
    locale,
  });
}
...
export default async function AcheterPage({
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-8 lg:px-8">
      <h1 className="font-serif text-3xl">Acheter un bien immobilier</h1>
      <p className="mt-2 text-charcoal/60">
        {hasLocation
          ? `${total.toLocaleString("fr-MA")} annonces · ${LISTINGS_PAGE_SIZE} par page`
          : "Sélectionnez une région, une ville et un quartier pour des résultats rapides."}
      </p>
      ...
      {hasLocation ? (
        items.length > 0 ? (
          ...
          <div className="mt-12 space-y-4 text-center">
            <p className="text-charcoal/60">Aucune annonce de vente pour ces critères.</p>
